                    ...
                    <div className={"video_class_description mt-15"}>
                            <div className={"video_class_description_details"}>
                                <div className={"video_class_description_details_first_col"}>
                                    <h1>Demo class of Hindi ({_readableTimeFromSeconds(timerTimeInterval)})</h1>
                                    <div className={"mt-15"}>
                    ...
                    {(chatModuleCurrentCallGroupMembers?.map((groupMembers,key)=>(
                        (!groupMembers?.isTeacher) ?
                            <span key={key} data-obj={JSON.stringify(groupMembers)}>
                                <video key={key}
                                       muted={(groupMembers?.mute && studentAllClassesList?.classData?.id !== groupMembers?.id) ? false : true}
                                       playsInline={true}
                                       autoPlay={true}
                                       data-muted={(groupMembers?.mute && studentAllClassesList?.classData?.id !== groupMembers?.id) ? false : true}
                                       id={groupMembers?.id}
